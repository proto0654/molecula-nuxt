...
- [x] Stable mobile canvas: stage/chrome on `100svw`×`100svh` (HUD fits with URL bar up); drawing buffer from canvas client box; `visualViewport` only refreshes pointer rect (no `setSize` on browser chrome toggle)
