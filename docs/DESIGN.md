...
- Blurbs: lowercase techno one-liners prefixed `// ` in the 3D typewriter; on mobile, break onto a second line at the content ` / ` separator. Part 1 = section context (`hero_blurb`). Part 2 = Options verb + CTA tail via [`navBlurbCta`](../app/lib/navigation/buildAtomBlurb.ts): after `emptyToNull` / merge `trim`, tails starting with `,.;:!?…` glue directly (`кликай, чтобы открыть кейсы`); otherwise a space is inserted (`click to pick…`). Author infinitive tails as `, чтобы …` + clear object; **hub leaves CTA empty** (`buildAtomBlurb` also skips verb for `home`). Seed: [`HERO_WP_FIELDS.md`](HERO_WP_FIELDS.md).
- Page titles (`SiteScrambleTitle`): measure locks layout; display paints. SSG HTML keeps display empty until hydrate (`.is-hydrated`) so measure+display do not concatenate in source (`ОБО МНЕОБО МНЕ`). Accessible name is `aria-label`.
