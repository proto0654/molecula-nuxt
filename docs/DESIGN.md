...
- Blurbs: lowercase techno one-liners prefixed `// ` in the 3D typewriter; on mobile, break onto a second line at the content ` / ` separator. Part 1 = section context (`hero_blurb`). Part 2 = Options verb + CTA tail via [`navBlurbCta`](../app/lib/navigation/buildAtomBlurb.ts): after `emptyToNull` / merge `trim`, tails starting with `,.;:!?…` glue directly (`кликай, стек…`); otherwise a space is inserted (`click to pick…`). Author `hero_blurb_cta` as `, …` on **every** nav page (incl. hub), target **16–18** characters. Seed: [`HERO_WP_FIELDS.md`](HERO_WP_FIELDS.md).
