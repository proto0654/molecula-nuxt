...
- Blurbs: lowercase techno one-liners prefixed `// ` in the 3D typewriter; on mobile, break onto a second line at the content ` / ` separator. Part 1 = section context (`hero_blurb`). Part 2 on navigable atoms = Options verb + CTA tail via [`navBlurbCta`](../app/lib/navigation/buildAtomBlurb.ts): after `emptyToNull` / merge `trim`, tails starting with `,.;:!?…` glue directly (`кликай, узнать…`); otherwise a space is inserted (`click to pick…`). Prefer grammatical tails (not button labels). Hub atom — description only, no CTA. Seed: [`HERO_WP_FIELDS.md`](HERO_WP_FIELDS.md).
