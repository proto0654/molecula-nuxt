...
Off-home, header route menu commits the URL immediately (`transitionTo`); the page overlay stays hidden (`is-awaiting-pose`) until the molecule pose settles. On home the two-step gesture is unchanged (hover preview, first click commit+focus+prefetch, second click `Navigator.navigateTo`). Second click on already-committed Home is a no-op. Home `navigateTo` runs **parallel** approach tweens: zoom+fill (`power1.inOut`, full `APPROACH_DURATION` when work remains), framing → `CENTER_FRAMING`, and peripheral **orbit sweep** (2π, linear). Orbit is a direct quaternion write from progress, not lagged slerp. Packing dolly + orbit into one shared ease caused hitch-time zoom snaps.
