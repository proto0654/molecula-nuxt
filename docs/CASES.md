...
| [`ArchiveDetailNav.vue`](../app/components/archive/DetailNav.vue) | Shared NEXT footer: `case-nav` tokens. With `sectionIndex` — numbered marker (`NN / См. также`) + case-style 50/50 rail (portfolio **and** services). Without — `case-nav--archive` decorative rail. Service next/prev labels: `services_nav_*` (fallback «Следующая/Предыдущая услуга»). |
...
| Case `/portfolio/[slug]` | SiteChrome | `CASE / NN` + Index → archive (restored page; header INDEX uses `transitionTo`, not full reload) |
