...
- WordPress REST: `NUXT_PUBLIC_WP_API_BASE` → `runtimeConfig.public.wpApiBase` (generate + `nuxt dev` only; production client is payload-only SSG)
...
- Brand accent: `--color-accent` / `ACCENT_COLOR` `#6ec99f` (logo) — HUD cues, listing hovers, committed reticle, scramble glow — [`docs/DESIGN.md`](docs/DESIGN.md)
...
**Persistent molecular shell is in:** canvas / controller live in the default layout across routes. Home is the only interactive spatial mode: hub `C` is always committed with full readout (typewriter blurb + USP), never an empty deselect. **Molecule hero marketing copy** (RU/EN `hero_*`) lives on the five WP nav pages — seed + live CMS refreshed 2026-09-19; typewriter tails are grammatical after `{кликай|тапай}` (`navBlurbCta` re-spaces after trim). **Orphan prepositions:** [`fixOrphanPrepositions`](app/domain/wp/fixOrphanPrepositions.ts) glues short RU/EN particles with NBSP on USP, case/service titles, offer titles, and detail prev/next — apply after `stripTags` ([`docs/CONTENT.md`](docs/CONTENT.md)). Prod is strict SSG — WP edits need generate deploy. **Pointer spin** (desktop): orbit-bounded great-circle roll toward the cursor via [`pointerSpin.ts`](app/lib/molecular/pointerSpin.ts); pauses on peripheral atom hover. **Dual-state hover preview** (while committed): pulsing reticle, bright title, accent wireframe, **flowing hub bond dashes**, and desktop SVG connector follow the preview atom without stealing focus. **Home idle autoplay** cycles committed nav items with a slide progress bar; while dwelling, the next atom gets a pulsing accent wireframe plus **bond dash flow** (hub → next, phase-locked to the wireframe pulse). Pauses on interaction. Other routes freeze the molecule and overlay page content after pose settle (`is-awaiting-pose`); settled freeze also lerps selection chrome (rings / cross / wireframe) to black. Leave-home approach: parallel zoom/fill + orbit (not one packed tween). `/about`, `/services` (plus `/services/:slug`), and `/contact` are WP-backed SSG (payload at generate). Foundation gaps closed. **EN i18n:** `/en/*` routes, `useLocale()`, `*_en` resolve in normalizers, `LocaleSwitch`, hreflang + prerender pairs — [`app/domain/i18n/`](app/domain/i18n/). Spatial architecture: [`docs/SPATIAL.md`](docs/SPATIAL.md). Full TZ: [`tasks/HEADLESS_NUXT_TZ.md`](tasks/HEADLESS_NUXT_TZ.md).
