...
<template>
  <article class="service-offer" :id="offer.anchor">
    <h3 v-if="offer.title" class="service-offer__title">
      <a class="service-offer__anchor" :href="`#${offer.anchor}`">{{ offer.title }}</a>
    </h3>
    <div
      v-if="offer.textHtml"
      class="service-offer__text case-content__prose"
    ...
    <p v-if="offer.price" class="service-offer__price">
      ...
      <NuxtLink to="/contact" class="service-offer__cta">{{ ctaLabel }}</NuxtLink>
    </p>
  </article>
</template>
